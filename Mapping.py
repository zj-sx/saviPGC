...
def read_gene_map(path: str) -> Tuple[Dict[str, str], Dict[str, str]]:
    ...
    if df is None or df.shape[1] < 2:
        raise ValueError(f"Failed to read {path}. Tried delimiters and shapes: {tried}")
...
def run_sex_aware_scanvi_surgery(
    ...
    2) sex head 
    3) sex-aware gating
    4) low-confidence or high-entropy predictions -> Unknown

    return：
      - adata_query_latent (X=latent Z, obs=re)
      - surgery_model
    """
    ...
    # scvi-tools SCANVI.load_query_data
    ...
    print(
        f"[sex-aware gating]  {changed} / {proba_raw.shape[0]}  "
        f"({changed/proba_raw.shape[0]:.1%})；threshold={sex_thr}, softness coefficient={soft_eps}"
    )
...
def apply_pgc_marker_filter(
    ...
    print(f"[pgc_filter]  {n_forced_unknown}  '{unknown_label}' (threshold={threshold})")
...
# ============================================================
# 4) Sex-gating -> Sankey nodes/links
# ============================================================
